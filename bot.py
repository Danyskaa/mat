            ...
            if balance_response is not None:
                ...
                while tiket > 0:
                    ...
                    if play_status != 200:
                        ...
                        tiket -= 1
            print(f"{get_random_color()}{Style.BRIGHT}[ Rank ]: Checking Rank.. {Style.RESET_ALL}", end="\r" ,flush=True)
            time.sleep(2)
            rank_response = get_rank(token)
            if rank_response is not None:
                if rank_response['data'] is not None:
                    if rank_response['data']['isCreated'] == True:
                        current_rank = int(rank_response['data']['currentRank']['rank'])
                        curent_rank_name = rank_response['data']['currentRank']['name']
                        unused_stars = int(rank_response['data']['unusedStars'])
                        print(f"{get_random_color()}{Style.BRIGHT}[ Rank ]: Current Rank = {current_rank} | {curent_rank_name} | unused starts: {unused_stars} {Style.RESET_ALL}                    ", flush=True)
                        if unused_stars > 0:
                            print(f"{get_random_color()}{Style.BRIGHT}[ Rank ]: Upgrading.. {Style.RESET_ALL}", end="\r" ,flush=True)
                            time.sleep(3)
                            up_rank, up_rank_response = upgrade_rank(token, unused_stars)
                            if up_rank_response != 200:
                                print(f"{Fore.RED+Style.BRIGHT}[ Rank ]: Failed to upgrade rank! {Style.RESET_ALL}", flush=True)
                            else:
                                upgraded_rank = int(up_rank['data']['currentRank']['rank'])
                                upgraded_rank_name = up_rank['data']['currentRank']['name']
                                print(f"{get_random_color()}{Style.BRIGHT}[ Rank ]: Success upgrade to rank {upgraded_rank} | {upgraded_rank_name} {Style.RESET_ALL}                    ", flush=True)
                                if upgraded_rank > current_rank:
                                    time.sleep(2)
                                    sharetg, sharetg_response = share_story(token)
                                    if sharetg_response != 200:
                                        print(f"{Fore.RED+Style.BRIGHT}[ Rank ]: Failed to share story! {Style.RESET_ALL}", flush=True)
                                    else:
                                        print(f"{get_random_color()}{Style.BRIGHT}[ Rank ]: Success share story {Style.RESET_ALL}                    ", flush=True)
                    else:
                        print(f"{Fore.RED+Style.BRIGHT}[ Rank ]: Failed to get Rank! (never claim medal) {Style.RESET_ALL}", flush=True)
